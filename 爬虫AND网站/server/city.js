...
function getCity(city){
	mongoose.connect(`mongodb://localhost/guangzhou`)
	console.log(`爬虫开始运行了……`)
	console.log(`https://www.lagou.com/jobs/positionAjax.json?city=${city}&needAddtionalResult=false&pn=1`)
   ...
   console.log('collection removed') 
	...
	async.series({
		one:(cb) => {
			superagent.post(`https://www.lagou.com/jobs/positionAjax.json?city=${city}&needAddtionalResult=false&pn=1`)
					.send({
                        first:true,
						pn:1,
						kd:""
		...
		two:(cb) => {
                for (let i = 1; i <= Math.ceil(page / 15); i++) {
                	console.log(`https://www.lagou.com/jobs/positionAjax.json?city=${city}&needAddtionalResult=false&pn=${i}`)
                    urls.push(`https://www.lagou.com/jobs/positionAjax.json?city=${city}&needAddtionalResult=false&pn=${i}`)
                }
